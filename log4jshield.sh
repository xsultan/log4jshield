...
function fixJARs(){
	...
	if [[ -f "$file" ]]; then
		...
		if [ $? -eq 0 ]; then
			...
			echo -e "${BOLD}[+] ${YELLOW}[INFO] created a backup of the patched jar under $file.bak${ENDCOLOR}${ENDBOLD}\n"
		fi
	else
...
if [[ $(find . -type d -name "LOG4J-SHIELD-*") ]];then
	...
	then
		...
		select folder in ${FOLDERS[@]}; do test -n "$folder" && break; echo "${RED}${BOLD}[!]${ENDBOLD}${ENDCOLOR} >>> Invalid selection"; done
			while IFS= read -r file; do VULNERABLE_JARS[${#VULNERABLE_JARS[@]}]="$file"; done < $folder/VULNERABLE-JARs.txt
			for jarFile in ${VULNERABLE_JARS[@]}
				do
					echo ${jarFile}
					fixJARs '$jarFile'
				done
...
FILES=$(find ~+ -type f -name '*.[ejwa]ar' 2>/dev/null);
...
echo -e "\t${BOLD}THE TOTAL NUMBER OF SEARCHED FILES: \t ${TOTAL_FILES} ${ENDBOLD}"
